...
export default function PilgrimEditPage() {
  ...
  async function loadPilgrim() {
    ...
    const [{ data: p }, { data: h }] = await Promise.all([
      supabase.from("pilgrims").select("*").eq("id", id).single(),
      supabase.from("v_pilgrim_housing").select("*").eq("pilgrim_id", id).single(),
    ]);
  ...
  if (!pilgrim) {
    return <div className="text-center py-12 text-red-500">لم يتم العثور على الحاج</div>;
  }
      ...
      <header className="mb-6">
        ...
        {housing && (
          <p className="text-slate-500 mt-1 text-sm">
            الطابق {housing.floor_number} - غرفة {housing.room_number} - مجموعة{" "}
            {housing.group_number}
          </p>
        )}
      </header>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <Field
              ...
              onChange={(v) => setPilgrim({ ...pilgrim, full_name: v })}
              required
            />
            <Field
              label="رقم الهوية"
              value={pilgrim.national_id || ""}
              onChange={(v) => setPilgrim({ ...pilgrim, national_id: v })}
            />
            <Field
              label="رقم الجواز"
              value={pilgrim.passport_number || ""}
              onChange={(v) => setPilgrim({ ...pilgrim, passport_number: v })}
            />
            <Field
              label="رقم الهاتف"
              value={pilgrim.phone || ""}
              onChange={(v) => setPilgrim({ ...pilgrim, phone: v })}
              type="tel"
            />
            <Field
              label="البريد الإلكتروني"
              value={pilgrim.email || ""}
              onChange={(v) => setPilgrim({ ...pilgrim, email: v })}
              type="email"
            />
            <Field
              label="تاريخ الميلاد"
              value={pilgrim.birth_date || ""}
              onChange={(v) => setPilgrim({ ...pilgrim, birth_date: v })}
              type="date"
            />
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setPilgrim({ ...pilgrim, gender: e.target.value || null })
                }
            ...
            <Field
              ...
              onChange={(v) => setPilgrim({ ...pilgrim, nationality: v })}
            />
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <Field
              label="الاسم"
              value={pilgrim.emergency_contact_name || ""}
              onChange={(v) =>
                setPilgrim({ ...pilgrim, emergency_contact_name: v })
              }
            />
            <Field
              label="رقم الهاتف"
              value={pilgrim.emergency_contact_phone || ""}
              onChange={(v) =>
                setPilgrim({ ...pilgrim, emergency_contact_phone: v })
              }
              type="tel"
            />
          </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-100 p-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <textarea
                value={pilgrim.medical_notes || ""}
                onChange={(e) =>
                  setPilgrim({ ...pilgrim, medical_notes: e.target.value })
                }
                rows={3}
            ...
            <div>
              ...
              <textarea
                value={pilgrim.special_needs || ""}
                onChange={(e) =>
                  setPilgrim({ ...pilgrim, special_needs: e.target.value })
                }
                rows={2}
                className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:ring-2 focus:ring-emerald-500 outline-none text-sm"
              />
        ...
        <div className="flex justify-end">
          <button
            type="submit"
            disabled={saving}
            className="inline-flex items-center gap-2 bg-emerald-600 hover:bg-emerald-700 disabled:bg-slate-300 text-white px-6 py-2.5 rounded-lg font-medium transition"
          >
